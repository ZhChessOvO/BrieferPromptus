"""
Evaluate PSNR, SSIM, and LPIPS between color-corrected results and ground truth.

Data structure (colorupdate, 扁平目录):
  {COLORUPDATE_DIR}/{id:05d}.png
  - 例如: rank1_interval10_baseline_colorupdate/00000.png
...
# ===== Config =====
COLORUPDATE_DIR = "/root/autodl-tmp/sky/results/rank4_interval10_colorupdate"
...
def get_colorupdate_path(id_val):
    """Get path to the color-corrected image."""
    return os.path.join(COLORUPDATE_DIR, f"{id_val:05d}.png")


def get_gt_path(id_val):
    """Get path to the GT image."""
    return os.path.join(GT_DIR, f"{id_val:05d}.png")


def main():
    print("=" * 60)
    print("Evaluation (Color-Corrected Results vs GT): PSNR, SSIM, LPIPS")
    print(f"  ColorUpdate dir: {COLORUPDATE_DIR}")
    ...
    for id_val in range(TOTAL_IDS):
        result_path = get_colorupdate_path(id_val)
    ...
    print("\n" + "=" * 60)
    print("Summary (Color-Corrected vs GT)")
    print("=" * 60)
    if missing_files:
